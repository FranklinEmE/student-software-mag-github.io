...
public class Course {
   ...
   public void insertUpdateDeleteStudent(char operation, Integer id, String label, Integer hours){
        ...
        if(operation == 'i'){
            try {
                ps = con.prepareStatement("INSERT INTO course (label, hours_number)  VALUE (?,?)");
      ...
      if(operation == 'd'){
           ...
           int  YesOrNo = JOptionPane.showConfirmDialog(null, "The Scores Will Be Also Deleted", "Delete Scores", JOptionPane.OK_CANCEL_OPTION, 0);
       ...
       try {
        ...
        ResultSet rs = ps.executeQuery();
            ...
            ps = con.prepareStatement("SELECT * FROM course");
                  ...
                  Object[] row;
               //  LIKE? ("SELECT * FROM studentinfor WHERE CONCAT('first_name', 'last_name', 'phone', 'address')LIKE ?");
                  while(rs.next()){
                      row = new Object[3];
                      row[0] = rs.getInt(1);
