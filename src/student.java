...
public class student {
  ...
  public void insertUpdateDeleteStudent(char operation, Integer id, String fname, String lname,
        ...
        if(operation == 'i'){
            try {
                ps = con.prepareStatement("INSERT INTO STUDENTINFOR (first_name, last_name, sex, birthdate, phone, address)  VALUE (?,?,?,?,?,?)");
      ...
      if(operation == 'd'){
         ...
         int  YesOrNo = JOptionPane.showConfirmDialog(null, "The Scores Will Be Also Deleted", "Delete Student", JOptionPane.OK_CANCEL_OPTION, 0);
          if(YesOrNo == JOptionPane.OK_CANCEL_OPTION){
               ...
               try {
                ps = con.prepareStatement("DELETE FROM studentinfor WHERE id = ?");
                ps.setInt(1, id);
       ...
       if(operation == 'u'){
            try {
                ps = con.prepareStatement("UPDATE studentinfor SET first_name = ?, sex =?, birthdate = ?, phone = ?, address = ?, WHERE id = ?");
                ps.setString(1, fname);
                ps.setString(2, lname);
                ps.setString(3, sex);
                ps.setString(4, bdate);
   ...
   public void fillStudentJtable(JTable table, String valueToSearch) {
       ...
       PreparedStatement ps;
      try {
          ps = con.prepareStatement("SELECT * FROM studentinfor WHERE CONCAT(first_name, last_name, phone, address)LIKE ?");
